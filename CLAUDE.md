...
- **Monorepo**: Node.js with workspaces and concurrently for multi-service development
- **Frontend**: React 18 + TypeScript + Vite + Tailwind CSS + PostCSS
- **UI Components**: Modern design system with shadcn/ui-style components (Card, Button, Badge, etc.)
...
- **Proxy Service**: Express-based AI gateway with security middleware
- **AI Providers**: OpenAI SDK + Anthropic SDK + Google Generative AI
- **Security**: Microsoft Presidio + Custom Rule Engine + JWT authentication
- **Testing**: Jest + Vitest + Supertest + Playwright
- **Build Tools**: TypeScript compiler + Vite + ESLint + tsx (dev runner)
...
| Task | Agent | Notes |
|------|-------|--------|
| **React Component Development** | @react-component-architect | Use for all React UI work, hooks, and component architecture |
| **Tailwind CSS & Styling** | @tailwind-css-expert | Use for all Tailwind utility work, responsive design, dark mode |
| **Backend API Development** | @backend-developer | Use for Express routes, middleware, Prisma models, business logic |
| **API Design & Contracts** | @api-architect | Use for REST endpoint design, OpenAPI specs, request/response schemas |
| **Code Quality & Security** | @code-reviewer | MANDATORY for all PR reviews, security audits, code quality gates |
| **Performance Optimization** | @performance-optimizer | Use for bottleneck analysis, database query optimization, bundle size |
| **Documentation Updates** | @documentation-specialist | Use for README updates, API docs, architecture guides |
| **General Frontend Tasks** | @frontend-developer | Fallback for non-React specific frontend work |

### Usage Guidelines

#### For React/Frontend Development:
- **Component Creation**: '@react-component-architect build a SecurityRuleCard component with edit/delete actions'
- **Styling Work**: '@tailwind-css-expert make the dashboard responsive with dark mode support'
- **UI System Updates**: '@react-component-architect update the MetricCard component to support loading states'

#### For Backend Development:
- **API Endpoints**: '@backend-developer create POST /api/v1/proxy/chat endpoint with PII detection'
- **Database Work**: '@backend-developer add User preferences table with Prisma migration'
- **Security Features**: '@backend-developer implement rate limiting middleware for proxy routes'

#### For Architecture & Design:
- **API Planning**: '@api-architect design REST endpoints for audit log filtering and export'
- **Performance Issues**: '@performance-optimizer analyze slow dashboard queries and optimize'
- **Code Reviews**: '@code-reviewer review the security middleware implementation'

#### For Documentation:
- **Guides**: '@documentation-specialist update the deployment guide for Docker Compose changes'
- **API Docs**: '@documentation-specialist generate OpenAPI spec for the new audit endpoints'

### Project-Specific Context

This AIProxy project has several unique characteristics that agents should be aware of:

1. **Security-First Architecture**: All data processing happens locally with PII detection via Microsoft Presidio
2. **Multi-Provider Support**: Handles OpenAI, Claude, and Gemini APIs with unified interfaces  
3. **Modern UI System**: Uses a custom design system with Card, Button, Badge components and consistent theming
4. **Monorepo Structure**: Three main packages (frontend, backend, proxy) with shared types
5. **Real-Time Features**: Dashboard shows live metrics and audit logs with WebSocket potential
6. **Enterprise Focus**: Built for compliance, audit trails, and enterprise security requirements

### Development Workflow Integration

1. **Planning Phase**: Use @api-architect for endpoint design, @react-component-architect for UI planning
2. **Implementation**: Route to appropriate specialists (@backend-developer, @react-component-architect, @tailwind-css-expert)
3. **Quality Gate**: ALWAYS use @code-reviewer before merging to main branch
4. **Performance**: Use @performance-optimizer when adding resource-intensive features
5. **Documentation**: Use @documentation-specialist for user-facing changes and API updates
